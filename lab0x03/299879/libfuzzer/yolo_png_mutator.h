...
// A simple class for parsing, serializing, and mutating a YOLO PNG file.
...
// YOLO PNG inputs, you can turn that fuzz target into a structure-aware one
// by defining YOLOPNG_MUTATOR_DEFINE_LIBFUZZER_CUSTOM_MUTATOR and then
// including this file.
class YoloPngMutator {
  using V = std::vector<uint8_t>;

public:
  // Parse the input stream as a YOLO PNG file,
  ...
  YoloPngMutator(std::istream &in) {
    ...
    Read4(in); // ignore CRC
    ssize_t idat_idx = -1;
    bool got_palette = false;

    while (in) {
      uint32_t len = ReadInteger(in);
      uint32_t type = Read4(in);
      if (type == Type("IEND")) {
        break; // do nothing, go to next chunk
      }
      char chunk_name[5];
      ...
      if (type == Type("PLTE")) {
        ...
        break; // cannot read any other chunk
      }
  ...
  void Mutate(Mutator m, unsigned int Seed) {
    ...
    // Mutate IHDR.
    case 0:
      // we fully mutate the raw bytes
      m(ihdr_.data(), ihdr_.size(), ihdr_.size());
      // we use the known constraints to make sure we define a mutation that
      // passes checkpoints
      ihdr_[8] = 8;                    // bit depth
      ihdr_[9] = 3 + 3 * (rand() % 2); // 3 or 6 for color type
      ihdr_[10] = 0;                   // compression method
      ihdr_[11] = 0;                   // filter
      ihdr_[12] = 0;                   // interlace
      break;
